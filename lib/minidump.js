...
const commands = {
  ...
  dump_syms: (() => {
    if (process.platform === 'darwin') {
      const path1 = path.resolve(__dirname, '..', 'build', 'src', 'tools', 'mac', 'dump_syms', 'dump_syms_tools')
      if (fs.existsSync(path1)) {
        return path1
      }
      const path2 = path.resolve(__dirname, '..', 'build', 'src', 'tools', 'mac', 'dump_syms', 'dump_syms_mac')
      if (fs.existsSync(path2)) {
        return path2
      }
      throw new Error("dump_syms excecutable was not found for macOS")
